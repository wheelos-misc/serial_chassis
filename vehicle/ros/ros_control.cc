...
bool ROSControl::ProcessHeader() {
  ...
  if (ring_buffer_.Read(&header, 1) == 1) {
    ...
    if (header == 0x7B) {
      ...
      AINFO << "Invalid header: 0x" << HexToString(&header, 1);
      return false;
    }
    return true;
  }
...
bool ROSControl::IsFrameChecksumValid(const std::vector<uint8_t>& frame_data) {
  constexpr size_t kMinFrameSize = 2;

  if (frame_data.size() < kMinFrameSize) {
    AERROR << "Invalid frame: too short!";
    return false;
  }

  const size_t data_length = frame_data.size() - 2;
  uint8_t checksum_calculated =
      std::accumulate(frame_data.begin(), frame_data.begin() + data_length,
                      static_cast<uint8_t>(0), std::bit_xor<uint8_t>());

  const uint8_t checksum_received = frame_data[data_length];

  if (checksum_calculated != checksum_received) {
    AINFO << "Checksum mismatch! Calculated: 0x" << std::hex
          << static_cast<int>(checksum_calculated) << " Received: 0x"
          << static_cast<int>(checksum_received);
    return false;
  }
